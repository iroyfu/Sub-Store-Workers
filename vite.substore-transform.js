    ...
    function precompilePeggyParser(contents, id, pluginContext) {
        ...
        if (!match) {
            ...
            pluginContext.error(
                '[sub-store-transform] ' +
                id +
                ' Peggy parser 预编译失败：未找到 grammar/grammars；文件开头：' +
                preview,
            );
            return null;
        }
        ...
        const output =
            '// __SUB_STORE_WORKERS_PATCH__PEGGY_PRECOMPILED_PARSER__\n' +
        ...
        if (output.includes('peggy.generate')) {
            pluginContext.error(
                '[sub-store-transform] ' +
                id +
                ' Peggy parser 预编译失败：生成结果仍包含 peggy.generate',
            );
            return null;
        }
        ...
        fallbackDone(response);
                ...
                if (contents !== before) {
                    ...
                    if (!contents.includes('dispatch(request, i + 1);')) {
                        this.error('[sub-store-transform] express.js next() 补丁自检失败');
                    }

                    if (!contents.includes('function Response(corsHeaders = {}, requestId) {')) {
                        this.error('[sub-store-transform] express.js Response 补丁自检失败');
                    }

                    if (!contents.includes('Response(cors.headers, request.__requestId)')) {
                        this.error('[sub-store-transform] express.js cors 响应补丁自检失败');
                    }
                } else {
                    this.error('[sub-store-transform] express.js 补丁未应用');
                }
            }
...
export class OpenAPI`,
                    ...
                    this.error('[sub-store-transform] open-api.js 补丁未应用：未找到 OpenAPI');
                }
                ...
                if (missing.length > 0) {
                    this.error(
                        '[sub-store-transform] open-api.js 补丁自检失败：缺少 ' +
                        missing.join(', '),
                    );
                }
            }
            ...
            if (id.includes('sub-store/backend/src/utils/download.js')) {
                ...
                if (!contents.includes('__SUB_STORE_WORKERS_PATCH__INFLIGHT_TASKS__')) {
                    ...
                    if (!contents.includes('const tasks = new Map();')) {
                        this.error('[sub-store-transform] download.js 缺少 tasks 定义');
                    }
...
const tasks = {
    ...
    delete: () => {},
                    ...
                    if (startIdx === -1 || endIdx === -1 || startIdx >= endIdx) {
                        this.error('[sub-store-transform] 无法定位 download() 边界：' + id);
                    }
                    ...
                    const missing = requiredNeedles.filter((needle) => !chunk.includes(needle));

                    if (missing.length > 0) {
                        this.error(
                            '[sub-store-transform] download.js 缺少关键片段：' +
                            missing.join(', '),
                        );
                    }
...
`;
                    ...
                    if (!contents.includes('__SUB_STORE_WORKERS_PATCH__INFLIGHT_TASKS__')) {
                        this.error('[sub-store-transform] download.js 补丁自检失败');
                    }
                }
            }
            ...
            if (id.includes('sub-store/backend/src/core/proxy-utils/processors/index.js')) {
                ...
                if (!contents.includes('__SUB_STORE_WORKERS_PATCH__QUICKJS_CREATE_DYNAMIC_FUNCTION__')) {
                    ...
                    if (startIdx === -1) {
                        this.error('[sub-store-transform] processors/index.js 未找到 createDynamicFunction');
                    }
...
`;
                    ...
                    if (!contents.includes('__SUB_STORE_WORKERS_PATCH__QUICKJS_CREATE_DYNAMIC_FUNCTION__')) {
                        this.error('[sub-store-transform] processors/index.js 补丁自检失败');
                    }
                }
            }
            ...
            if (id.includes('sub-store/backend/src/core/app.js')) {
                ...
                if (!contents.includes('__SUB_STORE_WORKERS_PATCH__OPENAPI_DEBUG__')) {
                    ...
                    if (contents !== beforeApp) {
                        ...
                        this.error('[sub-store-transform] core/app.js debug 补丁未应用');
                    }
                }
            }
...
`;
                ...
                if (contents.includes('jsrsasign')) {
                    this.error('[sub-store-transform] utils/rs.js 仍包含 jsrsasign');
                }

                if (!contents.includes("from 'node:crypto'")) {
                    this.error('[sub-store-transform] utils/rs.js 未使用 node:crypto');
                }
            }
        ...
        buildEnd() {
            ...
            for (const [name, seen] of requiredTargetFiles) {
                if (!seen()) {
                    this.error('[sub-store-transform] 必需补丁目标未进入构建图：' + name);
                }
            }
